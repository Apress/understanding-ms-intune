Function Invoke-PreInstallation {
    ...
    # Perhaps copy some files, register some DLLs or anything else you can think of!
}
...
Function Invoke-ApplicationInstall {
    ...
    Process { 
        ...
        if ($InstallBasedOnOfficeBitness) {
            ...
            switch ($obj.OSIs64Bit) {
                $True {start-process -FilePath "$ENV:SystemRoot\System32\msiexec.exe" -ArgumentList "/i "".\Your64-BitMSI"" /qn" -NoNewWindow -Wait}
                default {start-process -FilePath "$ENV:SystemRoot\System32\msiexec.exe" -ArgumentList "/i "".\Your32BitMSI"" /qn" -NoNewWindow -Wait}
            }
        }
    ...
    End {
        ...
        "Logged In User: $($Obj.CurrentUser)" | out-file -FilePath "$env:SystemRoot\temp\Invoke-ApplicationInstall.log" -Append -ErrorAction SilentlyContinue
        "Office Version is 32Bit: $($Obj.OfficeIs32Bit)" | out-file -FilePath "$env:SystemRoot\temp\Invoke-ApplicationInstall.log" -Append -ErrorAction SilentlyContinue
        "Operating System is 64Bit: $($Obj.OSis64Bit)" | out-file -FilePath "$env:SystemRoot\temp\Invoke-ApplicationInstall.log" -Append -ErrorAction SilentlyContinue
    }
}
